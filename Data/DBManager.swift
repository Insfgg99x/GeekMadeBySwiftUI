...
class DBManager: NSObject {
    ...
    private var ctx = NSManagedObjectContext(concurrencyType: .mainQueueConcurrencyType)
...
extension DBManager {
    ...
    func insert(feed: Feed?) {
        ...
        entity?.images = feed?.images?.string()
    }
    
    func entitys() -> [Favorites]? {
        let request: NSFetchRequest = Favorites.fetchRequest()
        let sort = NSSortDescriptor.init(key: "type", ascending: true)
        request.sortDescriptors = [sort]
        var array: [Favorites]? = nil
        ctx.performAndWait {
            do{
